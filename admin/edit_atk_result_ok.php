...
if (strlen($_SESSION['id'] == 0)) {
    ...
    $result_text;
    if ($result == "YES") {
        $result_text = "ติดเชื้อ (2ขีด)";
    } else {
        $result_text = "ไม่ติดเชื้อ (1ขีด)";
    }

    // echo $date,$time,$loca,$fac,$band,$howmany;
    $sql = "UPDATE `atk_test` SET `AT_ID`='$AT_ID',`atopen_id`='$atopen_id',
    `user_id`='$user_id',`Date`='$date',
    `result`=' $result_text' WHERE AT_ID = '$AT_ID'";
...
?>
    ...
    <body>
        ...
        border: none; padding: 50px; padding-top: 25px;">
                    <center>
                        <h1>อัพเดทข้อมูลผลตรวจ ATK เรียบร้อยแล้ว!</h1>
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function() {
                Swal.fire({
                    title: 'อัพเดทข้อมูลผลตรวจ ATK เรียบร้อยแล้ว!',
                    icon: 'success',
